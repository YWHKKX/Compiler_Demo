...
void display(struct node *T, int indent)
{
    ...
    if (T)
    {
        ...
        case TYPE:
            ...
            break;
        case EXT_DEC_LIST:
            display(T->ptr[0], indent); // ID || ARRAY_DEC
        ...
        case STRUCT_DEF:
            printf("%*c外部结构体定义：\n", indent, ' ');
            printf("%*c结构体名：%s\n", indent+3, ' ', T->ptr[0]->struct_name);
            display(T->ptr[1], indent + 3);
            break;
        case EXP_ELE:
        case EXP_ARRAY:
            break;
        case ARRAY_LIST: /* [Exp,ARRAY_LIST] */
            printf("%*cARRAY[%d]", indent+2, ' ', T->ptr[0]->type_int);
        ...
        case VAR_DEF:
            printf("%*c变量定义：\n", indent, ' ');
            display(T->ptr[0], indent + 3); // TYPE
            display(T->ptr[1], indent + 3); // DEC_LIST
            break;
        case DEC_LIST:
            printf("%*c变量：\n", indent, ' ');
            T0 = T;
        ...
        case ID:
            ...
            break;
        case IDS:
            printf("%*cID(pointer)： %s\n", indent, ' ', T->type_id);
            break;     
        case IDA:
            printf("%*cID(address)： %s\n", indent, ' ', T->type_id);
            break;          
        case ARRAY_DEC: /* [ID,ARRAY_LIST] || [ARRAY_DEC,ARRAY_LIST] */
            display(T->ptr[0], indent);
            display(T->ptr[1], indent);
            printf("\n");
            break;
