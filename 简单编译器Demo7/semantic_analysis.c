...
void ShowIR(struct node *T){
    ...
    for(i=0;i<symbolTable.index;i++){
        if(symbolTable.symbols[i].flag == 'a'){
            ...
            sprintf(msg,"ARRAY %s(%s)[%d]<+%d>\n", symbolTable.symbols[i].alias,check_type(symbolTable.symbols[i].type,type),sum,sum*get_typelen(symbolTable.symbols[i].type));
            print_io(msg,fd);
        }
        else if(symbolTable.symbols[i].flag == 'v'){
            sprintf(msg,"VAR %s(%s)<+%d>\n", symbolTable.symbols[i].alias,check_type(symbolTable.symbols[i].type,type),get_typelen(symbolTable.symbols[i].type));
            print_io(msg,fd);
        }
        else if(symbolTable.symbols[i].flag == 's'){
            sprintf(msg,"VAR %s(%s)<+%d>\n", symbolTable.symbols[i].alias,symbolTable.symbols[i].stname,symbolTable.symbols[i].offset);
            print_io(msg,fd);
        }
    }
}
...
void array_exp_tmp(struct node *T,struct node *T0,int key){
    struct opn opn1, opn2, result;
...
void array_exp_right(struct node *T,struct opn *opn1){
    ...
    if(array_exp(T,&T0,&sum,1)){
        (*opn1).kind = ARRAY;
        sprintf((*opn1).id,"%s(@%s[%d])<+%d>",symbolTable.symbols[T0->ptr[0]->place].alias,
            symbolTable.symbols[T0->ptr[1]->place].name,sum,
            symbolTable.symbols[T0->ptr[1]->place].offset+sum*get_typelen(symbolTable.symbols[T0->ptr[1]->place].type));
        T0->place = T0->ptr[1]->place;
        (*opn1).offset = symbolTable.symbols[T0->ptr[0]->place].offset-8;
    }
    else{
        (*opn1).kind = ARRAY;
...
void ids_exp_left(struct node *T,struct opn *result){
    ...
    if(symbolTable.symbols[T->ptr[0]->place].flag == 'p' || symbolTable.symbols[T->ptr[0]->place].flag == 'P'){
        ...
        semantic_error(T->pos, symbolTable.symbols[T->ptr[1]->place].name, "不是指针，语法错误");
    }
}
