...
%type  <ptr> program ExtDefList ExtDef Specifier StructDec StructDef OptTag Tag ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt ForDec ArrayList StmList DefList Def DecList Dec Exp Args

%token <type_int> INT              
%token <type_id> ID IDS IDA RELOP TYPE  
%token <type_float> FLOAT       
%token <type_char> CHAR       
%token <type_string> STRING    

%token STRUCT LP RP LB RB LC RC SEMI COMMA DOT  
%token PPLUS MMINUS PLUS MINUS ADDR STAR DIV ASSIGNOP MINUSASSIGNOP PLUSASSIGNOP DIVASSIGNOP STARASSIGNOP AND OR NOT IF ELSE WHILE FOR RETURN
...
StructDec: STRUCT Tag {$$=mknode(STRUCT_DEC,$2,NULL,NULL,yylineno);}
          ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}
      ...
      | Exp DOT Exp	{$$=mknode(EXP_ELE,$1,$3,NULL,yylineno);}
